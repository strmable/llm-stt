...
문서 버전: 3.2 (2026-07-12)
이 문서는 **자기완결적 최종본**이며, v3.2에서 실제 구현 코드(GUI + CLI 파이프라인, 2026-07-09 기준 end-to-end 동작 확인)와 문서를 동기화했다. 구현과 어긋났던 서술(포트, config 스키마, manifest 스키마, SRT 갱신 방식, 설정 창 구조 등)을 코드 기준으로 정정했다 — 상세는 부록 A "v3.1 → v3.2".
...
[Transcript] [완전 취소 (임시파일 삭제)] [Copy] [Settings]
...
```text
[ ] Text Correction (STT 결과를 문맥 기반으로 재교정) -- 차기 버전
...
* Text Correction(Phase C)은 [postprocessing.md](postprocessing.md)에 설계 완료, 구현은 차기 — 체크박스는 비활성 표시.
...
**Resume 안전장치 — `preserve_prior_transcriptions()`**: 이미 Stage 3을 거친 job에서 Phase A(chunk 재추출)를 같은 파라미터로 다시 돌리면 manifest가 새로 만들어지며 status가 전부 `vad_extracted`로 리셋될 수 있다. 이를 막기 위해 동일한 (start_sec, end_sec)의 chunk 중 `chunk_NNNN.txt`가 **실제로 디스크에 남아있는 것**만 `transcribed` 상태를 복원한다 — manifest의 낡은 status가 아니라 실물 파일 존재를 신뢰 소스로 삼는다(재전사 낭비 방지).
...
3. temp/{job_id}/manifest.json 존재 확인
   ...
   - 있음 → 다이얼로그: "이전 작업을 발견했습니다. 이어서 진행하시겠습니까?"
       [이어하기] → status가 transcribed가 아닌 chunk부터 Phase A/B 재개
                    (Phase A 재실행 시에도 §14.2의 preserve_prior_transcriptions로
                     이미 전사된 chunk는 보존)
