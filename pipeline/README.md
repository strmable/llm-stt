...
- ~~config.json의 VAD 기본값이 스크립트에 연결 안 됨~~ → 해결. `vad_raw_test.py`/`vad_merge.py`/
  ...
  `config.json`의 `provider`/`local_api`/`gemini`/`prompt` 등 나머지 섹션은 여전히 CLI 스테이지
  스크립트 자체는 읽지 않지만, 저장소 루트의 [`main.py`](../main.py)(PySide6 GUI, `gui/`)가 이제
  이 값들을 전부 읽어 Phase A/B를 구동한다 (design.md SS7-SS20) — `python main.py`로 실행.
