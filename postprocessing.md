...
문서 버전: 3.2 (2026-07-19)
이전 버전 대비 주요 변경: [부록 A](#부록-a-변경-이력) 참고. v3.2는 §11.1의 1차 분할(화자)/2차 분할(CPS)을 서로 다른 실행 시점(자동 Phase C vs 수동 SRT 후처리 도구)으로 분리했다 — [§14A](#14a-1차2차-분할-실행-시점-분리-v32-2026-07-19) 참고.
...
> **v3.2(2026-07-19) 실행 시점 분리.** 아래 §11.1의 1차 분할과 2차 분할은 더 이상 한 번에 실행되지 않는다. 실제 워크플로우가 "자동 화자분리 → **외부 도구로 SRT 번역** → 길이 분할" 순서여야 하기 때문이다 — 번역 전에 미리 길이를 잘라두면 외부 번역 툴이 문장을 온전히 보지 못해 번역 품질이 떨어진다. 상세는 §14 이후 변경 이력 참고.
>
> * **1차 분할(화자, 자동)**: Phase B 종료 직후 `gui/worker.py`의 `_phase_c()`가 자동 실행. `cue_splitter.split_by_speaker_cues()`. CPS 분할은 하지 않는다.
> * **2차 분할(CPS, 수동)**: 사용자가 번역 완료된 SRT를 메인 화면 "후처리" 버튼(모달)이나 `pipeline/srt_postprocess.py` CLI로 직접 실행. `cue_splitter.split_long_cue()`.

### 11.1 처리 순서 (개념 흐름 — 실제로는 아래처럼 두 시점에 나뉘어 실행됨)
...
> **v3.2(2026-07-19) 스키마 변경**: `text_correction.cue_splitter`에서 `cps_threshold`/`max_cue_duration_sec`/`min_cue_duration_sec`를 제거했다(자동 파이프라인에서 더 이상 CPS 분할을 하지 않으므로). 같은 필드들은 최상위 `srt_postprocess` 섹션으로 옮겨, §11.1 2차 분할(수동 SRT 후처리 도구) 전용 설정이 됐다. `text_correction`과 별도 최상위 키인 이유는 이 도구가 `text_correction.enabled`/서버 설정과 무관하게 동작하는 독립 유틸리티이기 때문이다(LLM 호출 없음, §11).
>
> **v3.2.1(2026-07-19) 버그 수정 + 기본값 확정**: `max_cue_duration_sec`가 `_allocate_timestamps()`에서 "이미 분할된 조각의 표시 시간"만 clamp할 뿐, 그 자체로 분할을 유발하지 못하는 버그가 있었다 — CPS는 낮은데(느린 화자, 침묵 포함) 단순히 길게 이어지는 30초짜리 통짜 cue(예: 화자 마커 없이 병합된 다중 화자 chunk)가 전혀 분할되지 않는 사례로 발견됨. `cue_splitter._apply_cps_split()`이 이제 CPS 초과 **또는** `alloc_dur > max_cue_duration_sec`일 때도 분할을 유발한다(해당 piece 자신의 char/sec 비율로 분할 기준 문자 수 산출). 이에 맞춰 값 미정이었던 기본값도 `null` → **`7.0`**으로 확정했다(Netflix 등 자막 제작 관행의 "한 cue 최대 7초 노출" 기준과 일치).
>
> **v3.2.2(2026-07-19) `max_chars_per_cue` 신설**: CPS·duration 두 기준 모두 "시간당" 제약이라, 짧고 빽빽한 대사(예: 3초에 60자)는 둘 다 통과해버려 여전히 한 화면을 다 덮을 수 있었다 — "초당 문자수보다 한 화면에 표시하는 문자 수 자체가 중요하다"는 실사용 피드백으로 확인됨. `max_chars_per_cue`는 cue의 원문 글자 수 자체를 직접 제한하는 세 번째 독립 분할 기준이며(시간과 무관), 세 기준 중 어느 하나라도 걸리면 분할된다. 기본값 `40`은 한국어 자막 관행(2줄 × 약 20자)과 대략 맞춘 값 — 콘텐츠에 따라 후처리 모달에서 조정 가능.
...
**`cue_splitter` — §11.1 1차 분할(화자, 자동 Phase C)**

| 필드 | 기본값 | 근거 |
|---|---|---|
| `gap_sec` | `0.08` | **§14 구현 시 신설.** 분할로 생성된 인접 cue 사이에 둘 최소 대기시간(초). 화자 마커 분할로 한 chunk가 여러 cue로 쪼개질 때, 각 cue를 원래 배정 시간보다 이 값만큼 일찍 끝내 다음 cue와 화면상 붙어 보이지 않게 한다(자막 제작 관행상 통상 2프레임 안팎). 원래 청크 경계(다른 VAD chunk와의 사이)는 이미 VAD 무음 구간으로 분리되어 있으므로 대상이 아니다. 마지막 조각은 원래 chunk의 end_sec을 그대로 사용 |
| `show_speaker_label` | `false` | §11.3: 기본 미노출 원칙 그대로 |

**`srt_postprocess` (최상위) — §11.1 2차 분할(CPS, 수동 SRT 후처리 도구)**

v3.2(2026-07-19)부터 `text_correction.cue_splitter`가 아니라 이 최상위 섹션에서 관리된다(자동 파이프라인과 분리된 독립 도구이므로).

| 필드 | 기본값 | 근거 |
|---|---|---|
| `cps_threshold` | `15` | §11.2: 한국어/CJK 통상 권장 범위(12~17자/초)의 중간값. **정확한 채택값은 구현 단계 확정 대상**(§24 미결정 사항 참고) |
| `max_cue_duration_sec` | `7.0` | §11.2/v3.2.1: 자막 제작 관행의 "최대 노출 시간" 상한. CPS는 낮아도 단순히 길게 이어지는 통짜 cue를 강제로 추가 분할하는 트리거로도 쓰인다(위 v3.2.1 버그 수정 참고). `null`로 두면 이 트리거가 꺼져 기존처럼 CPS만으로 분할 여부를 결정한다 |
| `min_cue_duration_sec` | `null` | 위와 동일. 너무 짧은 cue 깜빡임 방지용이나 값 미정 |
| `max_chars_per_cue` | `40` | §11.2/v3.2.2: CPS·`max_cue_duration_sec`는 둘 다 "시간당" 제약이라, 짧고 빽빽한 대사는 통과해버려 화면을 다 덮을 수 있다. cue 원문 글자 수 자체를 직접 제한하는 세 번째 독립 분할 기준(시간 무관). 한국어 자막 관행(2줄 × 약 20자) 근사치 |
| `gap_sec` | `0.08` | 위 `cue_splitter.gap_sec`과 같은 역할이나, 완전히 다른 실행 시점(번역 후)에 별도로 쓰이므로 독립 필드로 둔다 |

### 12.4 구현 전 확정 필요 (design.md §24에 추가할 항목)

* `srt_postprocess.cps_threshold`의 정확한 채택값 (12~17자/초 범위 내 확정)
* `srt_postprocess.max_cue_duration_sec`는 v3.2.1에서 `7.0`으로 확정됨(위 §12.3 참고). `min_cue_duration_sec`는 여전히 구체적 초 단위 값 미정
...
3. **vision encoder VRAM 점유** — 세 조합 모두 16GB 중 여유 있게 들어감(총점유량, 텍스트 전용 로드 옵션 확인은 안 됐으나 실사용상 문제 없는 수준):
   ...
   | Q8_0 | bf16 | 11.0GB |
   | Q8_0 | q8_0 | 10.6GB |
4. **prefix 캐시 실동작** — **검증 완료 (§13.7).** 실제 파이프라인(`text_correction.py`) 구동 로그에서 confirmed.
...
Q8_0 + q8_0 KV캐시로 확정(VRAM 10.6GB/16GB, 여유 5.7GB). VRAM 여유가 충분히 확인됐으므로, §3 "권장 설정 요약" 표의 기본값을 Q6_K에서 **Q8_0로 상향해도 무방**하다(품질 우선 시). 32K 컨텍스트를 실제 자막으로 꽉 채웠을 때의 여유는 §13.7에서 15869토큰 케이스로 확인됨(여유 있음) — 다만 이는 32768 한도 대비 약 48% 수준이라, 상한에 더 가까운 초장편 케이스는 여전히 별도 확인 대상.

### 13.7 Prefix 캐시 실동작 검증 (§13.4 항목 4, `text_correction.py` 구동 로그)

실제 파이프라인(`ensure_llama_server` + `text_correction.correct_all`, §14 구현) 구동 중 관측된 `llama-server --metrics` 로그:

```
slot launch_slot_: id  0 | task 12594 | processing task, is_child = 0
slot print_timing: id  0 | task 12594 | prompt eval time =  137.86 ms /   533 tokens (0.26 ms/tok, 3866.33 tok/s)
slot print_timing: id  0 | task 12594 |        eval time =  735.67 ms /    58 tokens (12.68 ms/tok,   78.84 tok/s)
slot print_timing: id  0 | task 12594 |       total time =  873.53 ms /   591 tokens
slot print_timing: id  0 | task 12594 |    graphs reused =    11759
slot      release: id  0 | task 12594 | stop processing: n_tokens = 15869, truncated = 0
```

해석:

* `release`의 `n_tokens = 15869`가 이 슬롯이 들고 있는 전체 컨텍스트(프롬프트+생성 토큰) 길이. 생성 토큰이 58개(`eval time` 행)이므로 이번 요청의 프롬프트 자체는 대략 15869 − 58 ≈ **15811 토큰**.
* 그런데 `prompt eval time`이 새로 계산한 프롬프트 토큰을 **533개**로 보고한다. Prefix 캐시가 안 걸렸다면 이 값이 15811에 근접해야 한다.
* 즉 15811 − 533 ≈ **15278 토큰(전체 프롬프트의 약 96.6%)이 이전 요청의 KV 캐시를 그대로 재사용**되고, `<CURRENT>` 블록(현재 교정 대상 chunk, §8.3) 크기에 해당하는 533 토큰만 새로 forward pass를 돌렸다.
* 533 토큰짜리 재계산이 137.86ms(3866 tok/s)로 끝난 것도 이 수치와 일치 — 15811 토큰 전체를 다시 계산했다면 이보다 한 자릿수 이상 오래 걸렸을 것.

**결론: §8.3에서 의도한 "`<FULL_TRANSCRIPT>`(고정 prefix)는 캐시 재사용, `<CURRENT>`(가변 suffix)만 새로 eval" 구조가 실제로 동작한다.** §13.4 항목 4 "미검증"을 이 관측으로 검증 완료 처리한다. (`graphs reused = 11759`는 별도 지표인 CUDA 그래프 재사용 카운트로, prefix 캐시 자체의 증거는 아니지만 같은 방향의 반복-연산 스킵 신호.)

---

## 14. 구현 결과 (2026-07-12)

Phase C가 GUI 파이프라인에 실제로 연결됐다. 산출물:

* **`pipeline/text_correction.py`** — §6/§8 full-context 교정. `chunk_NNNN.txt`(raw, Phase B 산출물)를 그대로 원문 단위로 재사용하고(별도 `raw/`·`fixed/` 디렉토리를 새로 만들지 않음), 교정 결과를 같은 자리에 `chunk_NNNN.fixed.txt`로 저장한다(§5 "원본·교정본 이중 보존"을 기존 chunk 파일 단위에 적용). `.fixed.txt`가 이미 있는 chunk는 건너뛰어 재실행이 resume-safe하다. `max_segment_chars` 초과 시 §6 "긴 파일 처리"대로 `segment_split_count`개 대구간으로 분할한다.
* **`pipeline/cue_splitter.py`** — §11 Cue Splitter를 그대로 구현. `[[SPEAKER]]` 마커 분할 → CPS 초과 조각의 문장부호/공백 경계 기계적 재분할(경계를 못 찾으면 하드컷 — 공백이 없는 CJK 대비) → 글자 수 비율 기반 시간 재배분. LLM 호출 없음.
* **`pipeline/server_manager.py`**에 `adapt_text_correction_server_config()` 추가 — Phase C 서버 설정(`text_correction.server`, flat 구조)을 기존 `ensure_llama_server()`가 기대하는 `local_api`/`managed` 중첩 구조로 변환만 한다(생명주기 로직 자체는 재사용, §12.2 근거와 일치).
* **`gui/worker.py`**에 `_phase_c()` 신설, Phase B 완료 후 순차 실행. `text_enhancement.text_correction.enabled`가 꺼져 있으면 완전히 스킵(파이프라인 기본 동작 불변). 켜져 있으면: Phase B가 만든 1-chunk-1-cue SRT를 `{입력파일명}.raw.srt`로 먼저 보존 → 교정 서버를 Managed/External 모드로 기동(§3, §12.2와 동일한 Managed 생명주기 재사용) → chunk별 교정 호출 → 완료 후 cue splitter로 최종 SRT를 재생성해 `{입력파일명}.srt`를 덮어쓴다.
* **`gap_sec` 신설** (§12.2/§12.3) — 원 설계(§11)에는 없던 값으로, 분할로 새로 생긴 인접 cue 사이에 짧은 시각적 공백을 두기 위해 구현 단계에서 추가했다. 상세 근거는 §12.3 표 참고.

**의도적으로 미구현:**

* `full_context.glossary_assist` — §7에서 "표기 흔들림이 실측에서 관측되면"으로 조건부 보류한 항목. 아직 관측된 바 없어 그대로 보류.
* CLI 단계 스크립트로서의 독립 실행 편의(로그 diff 출력 등, §7 "교정 신뢰도 출력 / 수정 Diff 출력")는 향후 개선 목록에 남겨둔다. `text_correction.py`는 `python pipeline/text_correction.py <job_dir>`로 단독 실행은 가능하나, 상세 리포팅은 GUI 로그 수준에 머문다.

---

## 14A. 1차/2차 분할 실행 시점 분리 (v3.2, 2026-07-19)

실사용 워크플로우 확인 결과, ASR 직후 자동으로 화자분할+CPS 길이분할을 한 번에 끝내던 기존 방식은 번역 단계와 충돌한다: 외부 번역 도구에 넘길 SRT가 이미 짧은 조각으로 쪼개져 있으면 번역기가 문장 전체 맥락을 보지 못해 번역 품질이 떨어진다. 그래서 §11의 두 분할 단계를 서로 다른 실행 시점으로 분리했다.

```
Phase B 완료
   ↓
[Phase C 자동] 화자 전환 탐지(§10) + 1차 분할(화자 마커만, §11.1)
   → {입력파일명}.srt  (아직 CPS 분할 전 -- 문장이 온전함)
   ↓
[사용자] 이 SRT를 외부 번역 도구로 번역
   ↓
[SRT 후처리, 수동] 메인 화면 "후처리" 버튼(모달) 또는
   `python pipeline/srt_postprocess.py {translated}.srt`
   → 2차 분할(CPS 길이 초과분만, §11.1)
   → 같은 파일명으로 덮어씀 (원본은 {파일명}.srt.bak으로 백업)
```

구현 변경:

* **`pipeline/cue_splitter.py`** — 기존 결합 함수 `split_chunk_to_cues()`를 제거하고, 이미 있던 `_apply_cps_split()`/`_allocate_timestamps()` 헬퍼를 재사용해 `split_by_speaker_cues()`(1차, 화자 전용)와 `split_long_cue()`(2차, CPS 전용, 화자 마커 무관)로 나눴다.
* **`pipeline/build_srt.py`** — 기존 `srt_timestamp()`(쓰기)에 대응하는 `parse_srt_timestamp()`/`parse_srt()`(읽기)를 추가해, 외부에서 번역된 SRT를 다시 읽어들일 수 있게 했다.
* **`pipeline/srt_postprocess.py`** (신규) — 2차 분할만 수행하는 단독 실행 CLI/라이브러리. `text_correction.py`/`build_srt.py`와 같은 스타일(단독 실행 가능 + GUI에서 함수 재사용).
* **`gui/worker.py`의 `_phase_c()`** — 이제 `split_by_speaker_cues()`만 호출한다. CPS 분할은 더 이상 자동 파이프라인에 없다.
* **`gui/srt_postprocess_dialog.py`** (신규) — "SRT 후처리" 모달. 파일 선택 → 옵션(CPS/길이/gap) → 실행 → 로그 출력의 흐름(design.md §7/§8.4).
* **config 스키마** — `text_correction.cue_splitter`에서 CPS 관련 3개 필드를 제거하고, 최상위 `srt_postprocess` 섹션으로 옮김(§12.2/12.3 갱신 반영).
...
| # | 변경 | 근거 |
|---|---|---|
| 1 | §6/§8/§11 설계를 `pipeline/text_correction.py`·`pipeline/cue_splitter.py`로 구현, `gui/worker.py`에 Phase C(`_phase_c()`)로 연결 (§14) | 설계 문서 상태에서 실제 파이프라인 실행으로 전환 |
| 2 | `cue_splitter.cue_splitter.gap_sec` 필드 신설 (§12.2/§12.3) | 구현 중 "분할된 cue끼리 화면상 붙어 보이는" 문제가 설계에 명시적으로 다뤄지지 않아, 분할 시에만 적용되는 최소 대기시간 옵션을 추가 |
| 3 | Phase C 서버 설정 어댑터 `adapt_text_correction_server_config()` 신설 (`pipeline/server_manager.py`) | §12.2 서버 설정이 flat 구조라 `ensure_llama_server()`의 기존 `local_api`/`managed` 중첩 구조와 직접 안 맞음 — 변환만 하고 생명주기 로직은 그대로 재사용 |
