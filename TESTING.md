...
`tools/eval_language_hint.py` 실행 결과. FLEURS의 `ref` 텍스트는 구두점이 전부 빠져 있고(중국어는 글자마다 공백도 들어감) 모델 출력은 자연스러운 구두점을 포함하므로, 원본 CER(raw)은 실제 전사 오류가 아닌 이 표기 차이로 부풀려진다. 2026-07-09에 `cer()`에 정규화(구두점·공백 제거, `normalize()`)를 추가해 재계산한 값을 함께 기록한다:

| 파일 | 언어 | auto CER (raw) | auto CER (정규화) | 힌트 CER (정규화) | 비고 |
|---|---|---|---|---|---|
| ja_000 | ja | 0.065 | 0.000 | 0.000 | 구두점 차이뿐, 실질 오류 없음 |
| ja_001 | ja | 0.086 | 0.049 | 0.049 | 고유명사 오류(ヴェルサーチ→ベルサージ), 二人 표기차 |
| ja_002 | ja | 0.036 | 0.000 | 0.000 | 구두점 차이뿐, 실질 오류 없음 |
| zh_000 | zh | 0.045 | 0.000 | 0.000 | 구두점/글자별 공백 차이뿐, 실질 오류 없음 |
| zh_001 | zh | 0.143 | 0.057 | 0.057 | 고유명사 오류(科隆→克隆) + "主" 중복 |
| zh_002 | zh | 0.077 | 0.000 | 0.000 | 구두점 차이뿐, 실질 오류 없음 |
| ko_000 | ko | 0.117 | 0.100 | 0.100 | "몰아냈음"→"보란했음"(의미 훼손) + 숫자 표기차 |
| ko_001 | ko | 0.136 | 0.091 | **0.000** | "무척"→"부쩍"(음운 유사 오류), 힌트로 정정됨 |
| ko_002 | ko | 0.140 | 0.105 | 0.105 | "족히"→"조기"(음운 유사 오류) + 숫자 표기차 |
| **평균** | | **0.094** | **0.045** | **0.035** | |
| ja 평균 | | 0.062 | 0.016 | 0.016 | |
| zh 평균 | | 0.088 | 0.019 | 0.019 | |
| ko 평균 | | 0.131 | 0.099 | 0.068 | |

**언어 오판: auto 0/9, 힌트 0/9.**

해석:
- 정규화 후 ja/zh는 6개 중 4개가 실질 오류 0%. 남는 오류는 전부 고유명사·이표기 수준으로, 낭독체 조건에서는 매우 우수한 성능.
- ko는 정규화해도 평균 9.9%(auto)로 ja/zh보다 확연히 높음 — 3건 모두 음운적으로 유사한 단어로의 실질 오인식(몰아냈음→보란했음, 무척→부쩍, 족히→조기)이 원인이며, 이는 정규화로 사라지지 않는 진짜 STT 오류. 한국어가 다른 두 언어보다 유의미하게 약하다는 신호.
- 9개 중 8개는 auto/힌트 출력이 완전히 동일 — 문맥이 충분한 **긴 자연문장**에서는 언어 힌트의 영향이 거의 없다. 세 언어 모두 힌트 없이도 100% 정확히 감지됨.
- 유일하게 달라진 `ko_001`은 힌트로 음운 유사 오류가 정정된 사례(정규화 CER 0.091→0.000) — 힌트 추가 비용은 사실상 0이고 손해 사례가 없으므로, 드물게 발생하는 이런 케이스에 대한 안전장치로 `language` 설정(§5A.8)을 유지하는 것이 합리적.
- 숫자 표기 차이("이백만" vs "200만", "1000년" vs "천년")는 발음이 동일해 실질 오류가 아니지만, 규칙 기반 정규화가 어려워 이번엔 반영하지 않고 수동 확인으로 분류했다(ko_000, ko_002에 남아 있음 — 이를 감안하면 ko 실질 CER은 표의 수치보다 조금 더 낮을 수 있음).
- CER 절대값(정규화 후 2~10%)은 [design.md](design.md#L165) §5A.6 콜센터 노이즈 데이터 벤치마크(22.72%)보다 훨씬 낮음 — Fleurs는 깨끗한 낭독체 음성이라 더 쉬운 조건. **노이즈/배경음악이 섞인 실사용 조건에서 격차가 얼마나 벌어지는지는 미검증** (§3.1 다음 단계).
